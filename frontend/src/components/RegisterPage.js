...
export default function RegisterPage({ onSwitchToLogin, onBackToHome }) {
  ...
  const getStrength = (pwd) => {
    ...
    if (s <= 1) return { level: s, label: "Weak", color: "#ef4444" };
    if (s <= 3) return { level: s, label: "Fair", color: "#f59e0b" };
    return { level: s, label: "Strong", color: "#10b981" };
  };
  ...
  return (
    <div style={{
      ...
      <style>{`
        ...
        .rp-card{animation:fadeIn .5s cubic-bezier(0.16,1,0.3,1) forwards}
        .rp-btn{transition:all .2s ease;cursor:pointer}
        .rp-btn:hover:not(:disabled){transform:translateY(-1px);box-shadow:0 10px 30px rgba(6,182,212,0.45)!important}
      ...
      <div className="rp-card" style={{
        display: "flex", width: "100%", maxWidth: "1080px", minHeight: "640px",
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", padding: "20px 40px 24px" }}>
            ...
            <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              ...
              <div style={{ position: "relative" }}>
                <input id="register-name" type="text" name="name" placeholder="Full Name"
              ...
              {/* Email */}
              <div style={{ position: "relative" }}>
                <input id="register-email" type="email" name="email" placeholder="Email Address"
              ...
              {/* Password */}
              <div style={{ position: "relative" }}>
                <input id="register-password" type={showPass ? "text" : "password"} name="password" placeholder="Password (at least 6 chars)"
              ...
              {form.password && (
                <div style={{ display: "flex", alignItems: "center", gap: "8px", marginTop: "-4px" }}>
                  <div style={{ display: "flex", gap: "4px", flex: 1 }}>
                    {[1, 2, 3, 4, 5].map(i => (
                      <div key={i} style={{ height: "3px", flex: 1, borderRadius: "2px", background: i <= strength.level ? strength.color : "#e5e7eb", transition: "background .3s" }} />
                    ))}
                  </div>
                  <span style={{ fontSize: "0.72rem", fontWeight: "700", color: strength.color, minWidth: "40px" }}>{strength.label}</span>
                </div>
              )}

              {/* Confirm Password */}
              <div style={{ position: "relative" }}>
                <input id="register-confirm" type={showPass ? "text" : "password"} name="confirm" placeholder="Confirm Password"
                  value={form.confirm} onChange={handleChange} required autoComplete="new-password"
                  onFocus={() => setFocused("confirm")} onBlur={() => setFocused(null)}
                  style={{
                    ...inputStyle("confirm"),
                    border: `1.5px solid ${form.confirm ? (form.confirm === form.password ? "#10b981" : "#ef4444") : (focused === "confirm" ? "#06b6d4" : "#e5e7eb")}`,
                  }} />
              </div>
              ...
              {/* Submit */}
              <button type="submit" id="register-submit-btn" className="rp-btn" disabled={loading}
                style={{
                ...
                {loading
                  ? <><span style={{ width: "16px", height: "16px", border: "2px solid #d1d5db", borderTopColor: "#6b7280", borderRadius: "50%", animation: "spin .7s linear infinite", flexShrink: 0 }} />Creating account...</>
                  : <>
